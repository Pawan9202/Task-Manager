/**
 * Database Seed Script — populates demo users, projects, and tasks
...
const seed = async () => {
  try {
    ...
    // Create demo users (passwords are hashed by User model pre-save hook)
    const admin = await User.create({ name: 'Admin User', email: 'admin@demo.com', password: 'admin123', role: 'admin' });
    const member = await User.create({ name: 'Member User', email: 'member@demo.com', password: 'member123', role: 'member' });
    const member2 = await User.create({ name: 'John Developer', email: 'john@demo.com', password: 'member123', role: 'member' });
    ...
    // Create tasks — mix of statuses, priorities, and overdue dates
    await Task.insertMany([
      {
        title: 'Design Homepage Layout',
        description: 'Create wireframes and high-fidelity mockups for the new homepage',
        status: 'done', priority: 'high',
        project: project._id, assignedTo: member._id, createdBy: admin._id,
      ...
      },
      {
        ...
        status: 'done', priority: 'high',
        project: project._id, assignedTo: member2._id, createdBy: admin._id,
      ...
      },
      {
        ...
        status: 'todo', priority: 'medium',
        project: project2._id, assignedTo: member2._id, createdBy: admin._id,
    ...
    Created: 2 Projects, 7 Tasks (2 done, 2 in-progress, 3 todo, 1 overdue)
    `);
